    ...
    create() {
        return {
            ClassDeclaration(node) {
                const injectedProp = node.body.body.find((body) => body.type === TSESTree.AST_NODE_TYPES.PropertyDefinition &&
                    body.key.type === TSESTree.AST_NODE_TYPES.Identifier &&
                    body.key.name === 'inject');
                const injectedConstructor = node.body.body.find((body) => body.type === TSESTree.AST_NODE_TYPES.MethodDefinition &&
                    body.key.type === TSESTree.AST_NODE_TYPES.Identifier &&
                    body.key.name === 'constructor');
                const injectedServices = injectedProp
                    ? findInjectedServices(injectedProp)
                    : [];
                const injectedArts = injectedConstructor
                    ? findInjectedArgs(injectedConstructor)
                    : [];
...
function findInjectedServices(element) {
...
function findInjectedArgs(element) {
        ...
        return element.value.params.map((param) => {
            if (param.type === TSESTree.AST_NODE_TYPES.Identifier && param.typeAnnotation) {
                if (param.typeAnnotation.typeAnnotation.type === TSESTree.AST_NODE_TYPES.TSTypeReference) {
                    if (param.typeAnnotation.typeAnnotation.typeName.type === TSESTree.AST_NODE_TYPES.Identifier) {
